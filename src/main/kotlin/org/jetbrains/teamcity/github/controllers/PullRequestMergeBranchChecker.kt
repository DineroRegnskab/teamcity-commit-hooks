...
class PullRequestMergeBranchChecker(
    ...
    inner class Action(val info: GitHubRepositoryInfo, val hook: WebHooksStorage.HookInfo, val user: UserEx, private val prNumber: Int) : Runnable {
        ...
        private fun onSucceed() {
            ...
            try {
                val response = RestApiFacade.request("POST", user, "text/plain", "/app/rest/vcs-root-instances/commitHookNotification", "locator=(type:jetbrains.git,count:1000),or:(property:(name:url,value:$httpId,matchType:contains),property:(name:url,value:$sshId,matchType:contains)),count:1000", emptyMap())
                if (response == null) {
